...
//ni buat buat ngambil /milih data dr database
    ...
    $sql = mysqli_query($koneksi,"SELECT transaksi.no_pelanggan , transaksi.tgl_transaksi, MAX(harga_transaksi) AS max_hrg_trans, MAX(tgl_transaksi) AS max_tgl_trans, COUNT(tgl_transaksi) as jumlah_transaksi 
		FROM transaksi, pelanggan WHERE transaksi.no_pelanggan = pelanggan.no_pelanggan and YEAR(transaksi.tgl_transaksi) = $tahun 
    ...
    //Recency
        if($data['tgl_transaksi'] > date('Y-m-d', strtotime('-18 months', strtotime( date('2020-02-d')))) ){
            $rec = 5;
        }else if($data['tgl_transaksi'] > date('Y-m-d', strtotime('-20 months', strtotime( date('2020-02-d')))) ){
            $rec = 4;
        }else if($data['tgl_transaksi'] > date('Y-m-d', strtotime('-22 months', strtotime( date('2020-02-d')))) ){
            $rec = 3;
        }else if($data['tgl_transaksi'] > date('Y-m-d', strtotime('-24 months', strtotime( date('2020-02-d')))) ){
            $rec = 2;
        }else 
        //if($data['tgl_transaksi'] > date('Y-m-d', strtotime('-26 months', strtotime( date('2020-02-d')))) )
        {
            $rec = 1;
        //} else {
        //    $rec = 0;
